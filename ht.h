...
{
    if (static_cast<double>(num_values)/ table_.size() > resizeAlpha_){
        resize();
    }    


    HASH_INDEX_T i = probe(p.first);  //probe with the key given
    if (i == npos){
        throw std::logic_error("can't find");
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    HASH_INDEX_T i = probe(key);
    if (i != npos && table_[i]!= nullptr){
...
{
    std::vector<HashItem*> prevElements = table_;
    mIndex_++;
    table_.clear();
    table_.resize(CAPACITIES[mIndex_], nullptr);
    num_values =0;
    for (int i = 0; i < prevElements.size(); i++){
        if (prevElements[i]!= nullptr){
            if (!prevElements[i]-> deleted){
                insert(prevElements[i]-> item);
            }
         
            delete prevElements[i];
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(/* Fill me in */ table_[loc]->item.first == key) {
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
